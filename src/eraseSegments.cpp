...
void eraseSegments(std::vector<uint_fast8_t>&Vec) {
	...
	const uint_fast32_t APP1_POS = searchFunc(Vec, 0, 0, APP1_SIG);
	if (Vec.size() > APP1_POS) {
		const uint_fast16_t APP1_BLOCK_SIZE = (static_cast<uint_fast16_t>(Vec[APP1_POS + 2]) << 8) | static_cast<uint_fast16_t>(Vec[APP1_POS + 3]);
		Vec.erase(Vec.begin() + APP1_POS, Vec.begin() + APP1_POS + APP1_BLOCK_SIZE + 2);
	}

	const uint_fast32_t APP2_POS = searchFunc(Vec, 0, 0, APP2_SIG);
